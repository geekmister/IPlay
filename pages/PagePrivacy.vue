<template>
  <div class="page-container">
    <h1 class="text-2xl font-bold mb-6">识别与隐私</h1>
    <p class="text-gray-600 mb-4">OCR、翻译、隐私保护与敏感区域处理</p>

    <div class="space-y-6">
      <div class="upload-box" v-if="!uploadedImage">
        <i class="fa fa-picture-o text-3xl text-gray-400 mb-2"></i>
        <p class="mb-1">上传图片</p>
        <p class="text-xs text-gray-500">用于 OCR 识别、翻译或隐私保护</p>
      ...
      <div v-else class="space-y-6">
          ...
          <div>
            ...
            <div class="bg-gray-50 border rounded-lg p-4">
              ...
              <button @click="processFeature" class="btn btn-primary w-full mt-4">
                <i class="fa fa-magic mr-1"></i>执行功能
              </button>
            </div>
          </div>
        </div>
        ...
        <!-- OCR 结果 -->
        <div v-if="selectedFeature === 'ocr' && ocrResult" class="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <h3 class="font-bold mb-2">OCR 识别结果</h3>
          <textarea v-model="ocrResult" class="w-full h-32 p-2 border rounded" readonly></textarea>
          <button @click="copyOcrResult" class="btn btn-secondary mt-2">
            <i class="fa fa-copy mr-1"></i>复制识别文本
          </button>
        </div>
...
const selectedFeature = ref('ocr')

const features = [
  { key: 'ocr', name: 'OCR 识别与翻译', desc: '识别图片文字并做简易翻译处理' },
  { key: 'privacy', name: '图片隐私保护', desc: '自动或手动马赛克敏感区域' }
]
...
const processFeature = () => {
  if (selectedFeature.value === 'ocr') {
    ocrResult.value = '文字识别功能开发中...\n[这里会显示识别到的文字]'
  } else if (selectedFeature.value === 'privacy') {
